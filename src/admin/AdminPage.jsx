...
function reorderCoursesByPriority(courses, priorityOrder) {
  const orderMap = new Map((Array.isArray(priorityOrder) ? priorityOrder : []).map((key, index) => [String(key || "").trim(), index]));
...
function movePriorityKey(order, draggedKey, targetKey) {
...
function handleSupportEmailClick() {
  try {
    if (!effectiveCollegeSlug) {
      showToast(
        "تعذر تجهيز الملف",
        "يبدو أن اسم الوحدة لم يُكتب بشكل صحيح، نأمل كتابته بدقة أو اختيار المدينة من صفحة التصدير وبوابة المتدربين.",
        "error"
      );
      return;
    }
...
const extraCandidates = extraPool
  ...
  function placeUnscheduledCourseInSlot(courseKey, targetSlotId) {
    ...
    setHallWarnings((prev) =>
      prev.filter((item) => {
        const warningKey = String(item?.courseKey || item?.key || "").trim();
        return warningKey !== String(course.key || "").trim();
      })
    );
  ...
  function restoreUnscheduledCourse(courseKey) {
    ...
    setHallWarnings((prev) =>
      prev.filter((item) => {
        const warningKey = String(item?.courseKey || item?.key || "").trim();
        return warningKey !== String(courseKey || "").trim();
      })
    );
...
const moveCoursePriority = (listType, draggedKey, targetKey) => {
  if (!draggedKey || !targetKey || draggedKey === targetKey) return;
  if (listType === "general") {
    setGeneralPriorityOrder((prev) => movePriorityKey(prev, draggedKey, targetKey));
    return;
  }
  setSpecializedPriorityOrder((prev) => movePriorityKey(prev, draggedKey, targetKey));
};

const nudgeCoursePriority = (listType, courseKey, direction) => {
  const setter = listType === "general" ? setGeneralPriorityOrder : setSpecializedPriorityOrder;
  setter((prev) => {
    const list = Array.isArray(prev) ? [...prev] : [];
    const index = list.indexOf(courseKey);
  ...
  const baseGeneralCourses = useMemo(
    () => parsed.courses.filter((course) => isGeneralStudiesCourse(course)),
    [parsed.courses]
  );

  const baseSpecializedCourses = useMemo(() => {
    const keys = new Set(baseGeneralCourses.map((c) => c.key));
    return parsed.courses.filter((course) => !keys.has(course.key));
  }, [parsed.courses, baseGeneralCourses]);

  useEffect(() => {
    setGeneralPriorityOrder((prev) => {
      const validKeys = baseGeneralCourses.map((course) => course.key);
      const validSet = new Set(validKeys);
      const preserved = (prev || []).filter((key) => validSet.has(key));
      const missing = validKeys.filter((key) => !preserved.includes(key));
      return [...preserved, ...missing];
    });
  }, [baseGeneralCourses]);

  useEffect(() => {
    setSpecializedPriorityOrder((prev) => {
      const validKeys = baseSpecializedCourses.map((course) => course.key);
      const validSet = new Set(validKeys);
      const preserved = (prev || []).filter((key) => validSet.has(key));
      const missing = validKeys.filter((key) => !preserved.includes(key));
      return [...preserved, ...missing];
    });
  }, [baseSpecializedCourses]);

  const generalCourses = useMemo(
    () => reorderCoursesByPriority(baseGeneralCourses, generalPriorityOrder),
    [baseGeneralCourses, generalPriorityOrder]
  );

  const specializedCourses = useMemo(
    () => reorderCoursesByPriority(baseSpecializedCourses, specializedPriorityOrder),
    [baseSpecializedCourses, specializedPriorityOrder]
  );
  ...
  const diagnoseUnscheduledCourse = (course) => {
    ...
    slots.forEach((slot) => {
      ...
      if (includeInvigilators) {
        ...
        const availableInvigilatorsCount = strictAvailableCount + extraAvailableCount;
...
const applySpecializedScheduleGeneration = ({
  ...
  setHallWarnings((prev) => {
    const remaining = (prev || []).filter((item) => {
      const warningKey = String(item?.courseKey || item?.key || "").trim();
      return !currentScopeKeySet.has(warningKey);
    });
    const combined = [...remaining, ...(nextHallWarnings || [])];
    const map = new Map();
    combined.forEach((item) => {
      const warningKey = String(item?.courseKey || item?.key || "").trim();
      const key = `${warningKey}__${item?.courseName || ""}__${item?.required || 0}__${item?.maxAvailable || 0}`;
      if (!map.has(key)) {
        map.set(key, item);
      }
    });
...
const matchesPreviewFilters = (item) => {
  ...
  const courseRef =
    parsed.courses.find((course) => String(course?.key || "").trim() === itemKey) || null;
...
}, [unscheduled, printDepartmentFilter, printMajorFilter, parsed.courses]);

const scheduledCourseKeySet = useMemo(() => {
  return new Set(
    (schedule || [])
      .map((item) => String(item?.courseKey || item?.key || "").trim())
      .filter(Boolean)
  );
}, [schedule]);
...
const filteredHallWarningsForPreview = useMemo(() => {
  return (hallWarnings || []).filter((item) => {
    const warningKey = String(item?.courseKey || item?.key || "").trim();
    if (warningKey && scheduledCourseKeySet.has(warningKey)) {
      return false;
    }

    return matchesPreviewFilters(item);
  });
}, [hallWarnings, scheduledCourseKeySet, printDepartmentFilter, printMajorFilter, parsed.courses]);
...
>
  ...
  >
    ...
    >
      ...
      >
        نظام احترافي لإنشاء جداول الاختبارات النهائية وتوزيع القاعات والمراقبين، 
        <br />
        مع أدوات متقدمة للمعاينة والطباعة والتصدير.
      </p>
    </div>
...
>
        ...
        {currentStep === 1 && (
          <Card>
            <SectionHeader
              ...
              description="ارفع تقرير SF01 أولًا، ثم أظهر فقط التفضيلات العامة التي تحتاجها. عند إلغاء أي خيار سيتم إخفاء إعداداته التفصيلية من الصفحات التالية لتبقى الواجهة أخف وأوضح."
            />
            ...
            <div style={{ marginTop: 22 }}>
              ...
              <div style={{ color: COLORS.muted, lineHeight: 1.9, marginBottom: 14 }}>
                أظهر فقط الخصائص التي تحتاجها. عند إلغاء أي خيار سيتم إخفاء بطاقته التفصيلية من الصفحات التالية حتى لا تتكدس الواجهة على المستخدم.
              </div>
  ...
  <SectionHeader
    ...
    description="يمكنك هنا إضافة القاعات الدراسية وتحديد الأقسام المسموح لها استخدام كل قاعة. كما يمكن تفعيل خيار مشاركة القاعة للقاعات ذات السعة الكافية فقط، بما يتيح إسناد أكثر من مقرر في الفترة الزمنية نفسها. علمًا بأنه لا يمكن إتمام عملية توزيع المقررات قبل استكمال إضافة القاعات."
  />
        ...
        >
          ...
          <span style={{ width: "100%", maxWidth: 600, textAlign: "right" }}>
            ...
            <textarea
              ...
              placeholder="يُترك فارغًا لسحب الأسماء تلقائيًا من التقرير، وفي حال الرغبة في الإدخال اليدوي، يمكن كتابة كل اسم في سطر مستقل"
...
{currentStep === 4 && (
          <Card>
            <SectionHeader
              ...
              description="يمكنك استبعاد المقررات غير المراد تضمينها ضمن عملية الجدولة.، كما يمكن أيضًا تحديد مستويات المقررات لمنع مقررات المستوى الواحد من الجدولة في نفس اليوم (الخصائص مخفية افتراضيًا ويمكن تفعيلها في صفحة التفضيلات)."
            />
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: COLORS.primaryLight }}>
                    {["الأولوية", "المقرر", "الرمز", "المدرب", "عدد المتدربين"].map((h) => (
                ...
                <tbody>
                  {generalCourses.map((course, index) => (
                    <tr
                      ...
                      onDragOver={(e) => e.preventDefault()}
                      onDrop={() => {
                        moveCoursePriority("general", draggingCourseKey, course.key);
                        setDraggingCourseKey("");
                      }}
                      onDragEnd={() => setDraggingCourseKey("")}
                      style={{
                        background: draggingCourseKey === course.key ? "#F8FEFE" : "#fff",
                        cursor: "grab",
                      }}
                    >
                      <td style={{ padding: 12, borderBottom: "1px solid #F1F5F9", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ minWidth: 28, fontWeight: 900, color: COLORS.primaryDark }}>{index + 1}</span>
                          <button type="button" onClick={() => nudgeCoursePriority("general", course.key, "up")} style={cardButtonStyle({ disabled: index === 0 })} disabled={index === 0}>↑</button>
                          <button type="button" onClick={() => nudgeCoursePriority("general", course.key, "down")} style={cardButtonStyle({ disabled: index === generalCourses.length - 1 })} disabled={index === generalCourses.length - 1}>↓</button>
                        </div>
                      </td>
                ...
                <tbody>
                  {specializedCourses.map((course, index) => (
                    <tr
                      ...
                      onDragOver={(e) => e.preventDefault()}
                      onDrop={() => {
                        moveCoursePriority("specialized", draggingCourseKey, course.key);
                        setDraggingCourseKey("");
                      }}
                      onDragEnd={() => setDraggingCourseKey("")}
                      style={{
                    ...
                    >
                      <td style={{ padding: 12, borderBottom: "1px solid #F1F5F9", whiteSpace: "nowrap" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ minWidth: 28, fontWeight: 900, color: COLORS.primaryDark }}>{index + 1}</span>
                          <button type="button" onClick={() => nudgeCoursePriority("specialized", course.key, "up")} style={cardButtonStyle({ disabled: index === 0 })} disabled={index === 0}>↑</button>
                          <button type="button" onClick={() => nudgeCoursePriority("specialized", course.key, "down")} style={cardButtonStyle({ disabled: index === specializedCourses.length - 1 })} disabled={index === specializedCourses.length - 1}>↓</button>
                        </div>
                      </td>
                      <td style={{ padding: 12, borderBottom: "1px solid #F1F5F9" }}>{course.courseName}</td>
                      <td style={{ padding: 12, borderBottom: "1px solid #F1F5F9" }}>{course.courseCode}</td>
...
{currentStep === 7 && (
  <Card>
    <SectionHeader
      ...
      description="اختر مقررين لعرض عدد المتدربين المشتركين بينهما مع بياناتهم."
    />
            ...
            <Card>
              ...
              <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginTop: 18 }}>
                ...
                <button
                  ...
                  style={cardButtonStyle()}
                >
                  {printOutputFormat === "docx" ? "حفظ جدول الاختبارات " : "طباعة جدول الاختبارات"}
                </button>
                ...
                <button
                  ...
                  style={cardButtonStyle()}
                >
                  {printOutputFormat === "docx" ? "حفظ جدول المراقبين " : "طباعة جدول المراقبين"}
                </button>
...
*/}
  ...
  <span style={{marginInlineStart: 6, color: COLORS.warning}}>
      ولإزالة بيانات البوابة بعد الانتهاء من الاختبارات، يجب حذف البيانات المحلية، ثم كتابة اسم الوحدة في صفحة الخصائص العامة بدون رفع التقرير ثم الضغط على زر النشر (نوصي بحفظ نسخة محلية لاستيرادها عند الحاجة)
  </span></div>
        {/*
          <div style={{ marginTop: 12 }}>
            <select
...
>
  ...
  <div>
                في حال عدم إتمام عملية النشر الآلي، يمكن اللجوء إلى خيار تصدير بيانات المتدربين محليًا، 
    <br /> ومن ثم رفع الملف لإكمال عملية النشر:  
           {/*  قسم الرفع */}
  <div style={{
     ...stepNineCardStyle,
  marginTop: 10,
  ...
  <CollegeJsonUploader />
    ...
    وعند عدم التمكن من ذلك، نأمل إرسال الملف المصدر بعد كل عملية توزيع على البريد التالي:
    <br />
